...
import { EmergencyAccessWithUser, EmergencyAccessReasonCode } from '@/types'
...
export default function EmergencyAccessPage() {
  ...
  const handleInvoke = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!patients) { setError('Patient not found'); setSubmitting(false); return }

      const expiresAt = new Date(Date.now() + 48 * 60 * 60 * 1000).toISOString()

      const { error: insertError } = await supabase.from('emergency_access').insert({
        doctor_id: authUser.id, patient_id: patients.id, reason_code: reasonCode, reason_text: reasonText, expires_at: expiresAt,
      })
  ...
  const handleRevoke = async (id: string) => {
    ...
    await supabase.from('emergency_access').update({ status: 'revoked' }).eq('id', id)
        ...
        <div className="glass-card animate-fade-in" style={{ padding: '32px', marginBottom: '32px', borderLeft: '4px solid #b71c1c' }}>
          ...
          <form onSubmit={handleInvoke} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div>
              ...
              <textarea rows={3} required value={reasonText} onChange={(e) => setReasonText(e.target.value)} placeholder="Describe the emergency situation..." className="input-field" style={{ resize: 'vertical' }} />
            </div>
